...
function LoadPlayer(identifier, playerId)
  ...
  table.insert(tasks, function(cb)
    MySQL.Async.fetchAll('SELECT `group`, loadout, position, inventory FROM users WHERE identifier = @identifier', {
      ...
      if result[1].position and result[1].position ~= '' then
        ...
        print(
            '[zrp_framework] [^3WARNING^7] Column "position" in "users" table is missing required default value. Using backup coords, fix your database.')
        userData.coords = {
          x = -269.4,
          y = -955.3,
          z = 31.2,
  ...
  Async.parallel(tasks, function(results)
    local zPlayer = CreatePlayer(playerId, identifier, userData.group, userData.inventory, userData.weight,
    ...
    print(
        ('[zrp_framework] [^2INFO^7] A player with name "%s^7" has connected to the server with assigned player id %s'):format(
            zPlayer.getName(), playerId))
  end)
end
...
AddEventHandler('chatMessage', function(playerId, author, message)
  if message:sub(1, 1) == '/' and playerId > 0 then
    ...
    TriggerClientEvent('chat:addMessage', playerId, {
      args = {_('system'), _U('commanderror_invalidcommand', commandName)}
    })
  end
end)
    ...
    if sourceZPlayer.hasWeapon(itemName) then
      ...
      if not targetZPlayer.hasWeapon(itemName) then
        ...
        if weaponObject.ammo and itemCount > 0 then
          ...
          sourceZPlayer.showNotification(_U('gave_weapon_withammo', weaponLabel, itemCount, ammoLabel,
              targetZPlayer.name))
          targetZPlayer.showNotification(_U('received_weapon_withammo', weaponLabel, itemCount, ammoLabel,
              sourceZPlayer.name))
        else
      ...
      if targetZPlayer.hasWeapon(itemName) then
        ...
        if weaponObject.ammo then
          ...
          if weapon.ammo >= itemCount then
            ...
            sourceZPlayer.showNotification(
                _U('gave_weapon_ammo', itemCount, ammoLabel, weapon.label, targetZPlayer.name))
            targetZPlayer.showNotification(_U('received_weapon_ammo', itemCount, ammoLabel, weapon.label,
                sourceZPlayer.name))
          end
        end
      else
